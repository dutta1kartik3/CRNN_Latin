...
function DatasetLmdb:nextBatch()
    ...
    for i = 1, self.batchSize do
        ...
        local imageByteLen = string.len(imgBine)
        local imageBytes = torch.ByteTensor(imageByteLen):fill(0)
        if (imageBytes ==  nil) then
        --or (imgBin == nil) then
       		print("2cvc2")
		goto startover
	end
	ib = imageBytes:storage()
	if (ib ==  nil) then
                goto startover
        end
	ib:string(imgBine)
        --if pcall(ib:string(imgBine)) then
	--	local ss = 256
	--else
	--	print("zxcv")
	--	goto startover
	--end
	local img = nil
        if pcall(Image.decompress, imageBytes, 3, 'byte') then
   		 img = Image.decompress(imageBytes, 3, 'byte')
	else
		--print(type(Image.decompress(imageBytes, 3, 'byte')))
		 goto startover
	end
        --local img = Image.decompress(imageBytes, 3, 'byte')
	if(img == nil) then
